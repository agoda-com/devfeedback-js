...
import type { Compilation, Compiler, Stats, StatsCompilation } from "webpack";
import webpack from "webpack";
import { type Server as WSServer, WebSocketServer } from "ws";

import type {
	DevFeedbackEvent,
	WebpackBuildData,
} from "@agoda-devfeedback/common";
...
export class WebpackBuildStatsPlugin {
	...
	apply(compiler: Compiler) {
		...
		compiler.hooks.watchRun.tap(
			...
			(compiler: Compiler) => {
				...
				const changedFiles =
					((compiler as any).modifiedFiles as string[]) || [];
		...
		compiler.hooks.done.tap("WebpackBuildStatsPlugin", async (stats: Stats) => {
			...
			const buildStats: WebpackBuildData = {
				...
				nbrOfCachedModules:
					jsonStats.modules?.filter((m) => m.cached).length ?? 0,
				nbrOfRebuiltModules:
					jsonStats.modules?.filter((m) => m.built).length ?? 0,
	...
	private recordEvent(partial: Omit<DevFeedbackEvent, "elapsedMs">) {
		...
		const fileNormalized = partial.file
			? this.normalizePath(partial.file)
			: undefined;
...
class DevFeedbackRuntimeModule extends webpack.RuntimeModule {
	...
	constructor(generateFn: () => string, wsServer: WSServer) {
